...
class LSHfaissSimilarity(object):
    ...
    def process_similarity(self, similarity):
        ...
        d = len(self._data.items)
        
        # Dictionary to store candidate sets for each user
        # candidate_neighbors[user_idx] = set of neighbor indices
        candidate_neighbors = {}
        
        X = self._URM.toarray().astype('float32')
        ...
        for table in range(self._n_tables):
            ...
            for user_idx, neighbors in enumerate(candidates):
                if user_idx not in candidate_neighbors:
                    candidate_neighbors[user_idx] = set()
                candidate_neighbors[user_idx].update(neighbors)
        ...
        for user, neighbors in tqdm(candidate_neighbors.items()):
            neighbors_list = list(neighbors)
            if len(neighbors_list) > 0:
                self._similarity_matrix[user, neighbors_list] = similarity_function(self._URM[neighbors_list], self._URM[user]).reshape(-1)
